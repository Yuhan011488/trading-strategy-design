...
# loading data
# train test sample sets split
part1 <- getData(directory="PART1")
# part2 <- getData(directory="PART2")
# part3 <- getData(directory="PART3")
# part2 <- lapply(part2, function(x) x[1:500])
# outSample <- lapply(part2, function(x) x[501:1000])

dataList <- part1

for(i in 1:10){
  dataList[[i]]<- rbind(part1[[i]], part2[[i]])
}

# for(i in 1:10){
#   dataList[[i]]<- rbind(dataList[[i]], part3[[i]])
# }
...
# store everyday orders and real money spent for each series
...
# **********************************
# Log Trade Signal
# record combined strategy trade signals
...
# Highest Close over holding period,
...
Strategy_Exit <- function(params, store){
  for (i in params$series){
    ...
    # stop trading particular series 
    # if strategy cumulatively loses more than 50k
    if (cum_pnl[i] < -50000){
      strategyExit_multiplier[i] <<- 0
    }
  }
